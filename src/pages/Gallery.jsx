        ...
        {visiblePhotos.length === 0 ? (
          ...
          <div className="p-2 space-y-2">
            ...
            >
              ...
              <img src={visiblePhotos[0].photo_url} alt={visiblePhotos[0].caption} loading="lazy" crossOrigin="anonymous" className="w-full h-full object-cover hover:scale-[1.02] transition-transform duration-300" onError={() => setBrokenImages(s => new Set([...s, visiblePhotos[0].id]))} />
              ) : (
                ...
                {visiblePhotos.slice(1).map((photo, idx) => (
                  ...
                  >
                    {photo.photo_url?.trim() && !brokenImages.has(photo.id) ? (
                      <img src={photo.photo_url} alt={photo.caption} loading="lazy" crossOrigin="anonymous" className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" onError={() => setBrokenImages(s => new Set([...s, photo.id]))} />
                    ) : (
        ...
        <Dialog open={!!selectedPhoto} onOpenChange={() => setSelectedPhoto(null)}>
          <DialogContent className="max-w-2xl w-full bg-black border-0 p-0">
            <div className="relative">
              {selectedPhoto?.photo_url?.trim() ? (
                <img src={selectedPhoto.photo_url} alt={selectedPhoto?.caption} crossOrigin="anonymous" className="w-full h-auto max-h-[80vh] object-contain" />
              ) : (
                <div className="w-full h-[80vh] flex items-center justify-center"><div className="text-gray-400">Unable to load image</div></div>
              )}
            ...
            return (
              <button className="w-full text-left" onClick={() => setSelectedAlbum(albums[0])}>
                <div className="relative w-full rounded-2xl overflow-hidden shadow-md" style={{ height: 220 }}>
                  ...
                  ? <img src={coverUrl} alt={albums[0].name} loading="eager" crossOrigin="anonymous" className="w-full h-full object-cover" onError={() => setBrokenImages(s => new Set([...s, albums[0].id]))} />
                    ...
                    <div className="absolute top-2 right-2 flex gap-1">
                      {heroPhotos.filter(p => p.photo_url?.trim()).map(p => (
                        <div key={p.id} className="w-10 h-10 rounded-lg overflow-hidden border-2 border-white/60 shadow">
                          <img src={p.photo_url} alt="" crossOrigin="anonymous" className="w-full h-full object-cover" />
                        </div>
                      ))}
                    </div>
                  )}
          ...
          {albums.length > 1 && (
            <div className="grid grid-cols-2 gap-3">
              {albums.slice(1).map(album => {
                const thumbs = album.cover_photo_url ? [] : (albumCovers[album.id] || []).slice(0, 3);
                ...
                return (
                  <button key={album.id} className="text-left" onClick={() => setSelectedAlbum(album)}>
                    <div className="relative rounded-xl overflow-hidden shadow-sm" style={{ height: 130 }}>
                      {coverUrl && !brokenImages.has(album.id)
                         ? <img src={coverUrl} alt={album.name} loading="lazy" crossOrigin="anonymous" className="w-full h-full object-cover" onError={() => setBrokenImages(s => new Set([...s, album.id]))} />
