      ...
      <header className="sticky top-0 z-50 w-full">
        <div className="bg-gradient-to-r from-[#1a237e] via-[#283593] to-[#3949ab] px-3 sm:px-4 pt-4 pb-5 shadow-lg">
          ...
          <div className="flex items-center justify-between w-full">
            ...
            {user && (
              <Link to={createPageUrl('UserProfile')} className="flex items-center gap-2 group px-2 py-1 rounded-lg hover:bg-white/10 transition-all">
                <div className="h-10 w-10 rounded-full bg-white/20 border-2 border-white/40 flex items-center justify-center flex-shrink-0 shadow group-hover:bg-white/30 transition-all">
                  <span className="text-white font-bold text-sm">{initials}</span>
                </div>
                <span className="text-white text-sm font-semibold">{user.full_name || user.name || user.email}</span>
              </Link>
            )}
          </div>
        </div>
