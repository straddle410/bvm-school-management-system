...
export default function Profile() {
  ...
  const loadProfile = async () => {
    try {
      ...
      // Fetch StaffAccount by email
      const accounts = await base44.entities.StaffAccount.filter({ email: staffUser.email });
      if (accounts.length > 0) {
        setStaffAccount(accounts[0]);
        setFormData({
          mobile: accounts[0].mobile || '',
          qualification: accounts[0].qualification || '',
          address_line1: accounts[0].address_line1 || '',
          address_line2: accounts[0].address_line2 || '',
          city: accounts[0].city || '',
          state: accounts[0].state || '',
          pincode: accounts[0].pincode || '',
          emergency_contact_name: accounts[0].emergency_contact_name || '',
          emergency_contact_phone: accounts[0].emergency_contact_phone || '',
          photo_url: accounts[0].photo_url || '',
        });
      }
    } catch (error) {
          ...
          <Card className="border-amber-200 bg-amber-50">
            <CardContent className="pt-6 flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">Your staff account could not be loaded.</p>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="max-w-2xl mx-auto">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid sm:grid-cols-3 gap-4">
                <div>
                  ...
                  {isEditing ? (
                    <Input name="city" value={formData.city} onChange={handleInputChange} className="mt-1" />
                  ) : (
                ...
                <div>
                  ...
                  {isEditing ? (
                    <Input name="state" value={formData.state} onChange={handleInputChange} className="mt-1" />
                  ) : (
                ...
                <div>
                  ...
                  {isEditing ? (
                    <Input name="pincode" value={formData.pincode} onChange={handleInputChange} className="mt-1" />
                  ) : (
                    <p className="mt-1 text-gray-900">{formData.pincode || '—'}</p>
                  )}
                </div>
              </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Cannot be changed</CardDescription>
            </CardHeader>
