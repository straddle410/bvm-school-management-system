...
Deno.serve(async (req) => {
  try {
    ...
    const { photoId, staffInfo } = await req.json();
    ...
    if (photo.status !== 'PendingApproval') {
      return Response.json({ error: `Cannot approve from ${photo.status} status` }, { status: 400 });
    }

    await base44.entities.GalleryPhoto.update(photoId, { status: 'Published' });
